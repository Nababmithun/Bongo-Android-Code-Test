...
class BadgeDrawable(context: Context) : Drawable() {
    ...
    init {
        val mTextSize = context.resources.getDimension(R.dimen.xSmallText)

        mBadgePaint = Paint()
        mBadgePaint.color = ContextCompat.getColor(context.applicationContext, R.color.redLight)
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (mCount.length <= 2) {
            // Draw badge circle.
            canvas.drawCircle(centerX, centerY, (radius + 8.5).toInt().toFloat(), mBadgePaint)
        } else {
            canvas.drawCircle(centerX, centerY, (radius + 9.5).toInt().toFloat(), mBadgePaint)
        }
